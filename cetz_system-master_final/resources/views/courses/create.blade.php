...
<label class="flex items-center gap-2">
    <input type="checkbox"
        ...
        data-section-id="{{ $dept->is_general 
            ? $dept->sections->first(function($s){
                return str_contains($s->name, 'عام');
            })?->id 
            : '' }}"

        @click="initSection($event.target.dataset.sectionId)"
...
function courseForm() {
    ...
    try {
        ...
        let response = await fetch("{{ route('courses.store') }}", {
            method: "POST",
            headers: {
        ...
        if (data.warning) {
            ...
            // إعادة الإرسال مع force
            formData.set('force', '1');

            let response2 = await fetch("{{ route('courses.store') }}", {
                method: "POST",
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json'
            ...
            // ❌ مهم جدًا: تحقق من النتيجة الثانية
            if (!response2.ok || data2.error) {
                alert(data2.message);
                return;
            }

            alert(data2.message || 'تم الحفظ بنجاح');
            return;
        }
