...
<div class="bg-white rounded-lg shadow p-4 space-y-4">
    ...
    <form method="GET" action="{{ route('deprivations.index') }}">
...
<div class="bg-white rounded-lg shadow p-4 space-y-4">
    ...
    <table class="min-w-full border text-sm">
        ...
        <tbody>
            @forelse($studentsEnrollments as $i => $enrollment)
                <tr>
                    <td class="border px-2 py-1">{{ $i + 1 }}</td>
                    <td class="border px-2 py-1">{{ $enrollment->student->full_name }}</td>
                    ...
                    <td class="border px-2 py-1">
                        <input type="text"
                            form="deprivationForm{{ $enrollment->id }}"
                            name="reason"
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center p-3">لا توجد نتائج</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <!-- pagination -->
    <div class="mt-3">
        {{ $studentsEnrollments->links() }}
    </div>
</div>

@else
    <div class="bg-yellow-100 text-yellow-800 p-3 rounded">
        الرجاء إدخال بيانات في الفلتر لعرض الطلاب
    </div>
@endif
```
    ...
    <tbody>
        @forelse($deprivations as $i => $d)
            <tr>
                ...
                <td class="border px-2 py-1">
                    <form action="{{ route('deprivations.destroy', $d->id) }}" method="POST"
                          onsubmit="return confirm('هل أنت متأكد من إزالة المحروم؟');">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="8" class="border px-2 py-1 text-center">
                    لا يوجد محرومين حتى الآن
                </td>
            </tr>
        @endforelse
    </tbody>
