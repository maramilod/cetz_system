...
<div>
  ...
  <select name="course_offering_id" x-model="selectedCourseOffering" class="w-full border rounded p-2">
    ...
    <template x-for="co in filteredCourses" :key="co.id">
      <option :value="co.id" x-text="co.name + ' - سيمستر ' + co.semester"></option>
    </template>
  </select>
</div>
...
<div>
  ...
  <select name="teacher_id" class="w-full border rounded p-2">
    @foreach($teachers as $t)
      @if($t->active) <!-- فقط المدرسين النشطين -->
...
function teachingForm() {
    return {
        selectedSection: '',
        selectedCourseOffering: '', // الآن هذا يمثل المادة + السيمستر المختار
        filteredCourses: [],
        allCourses: [
            @foreach($courseOfferings as $co)
        ...
        updateCourses() {
            ...
            // هنا نجمع كل نسخ المادة مع السيمستر مباشرة بدون فلترة التكرار على course_id
