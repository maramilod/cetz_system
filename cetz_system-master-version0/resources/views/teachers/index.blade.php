...
<div class="space-y-6" x-data="teachersList(@js($teachers))">
    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <form method="POST" action="{{ route('teachers.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
            ...
            <input type="text" name="national_id" placeholder="الرقم الوطني" class="border rounded px-3 py-2" required>
            ...
            <tbody>
                <template x-for="teacher in teachers" :key="teacher.id">
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border px-3 py-2" x-text="teacher.national_id"></td>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('teachersList', (initialTeachers) => ({
        ...
        newTeacher: { full_name: '', national_id: '', working_id: '', email: '', active: true },

        addTeacher() {
            if(!this.newTeacher.full_name || !this.newTeacher.national_id || !this.newTeacher.working_id || !this.newTeacher.email) return;
            const id = Date.now();
            this.teachers.push({ ...this.newTeacher, id });
            this.newTeacher = { full_name: '', national_id: '', working_id: '', email: '', active: true };
        },
  // تعديل أستاذ
        editTeacher(teacher) {
            const full_name = prompt("الاسم الكامل:", teacher.full_name);
            if(full_name === null) return;
            const national_id = prompt("الرقم الوطني:", teacher.national_id);
            if(national_id === null) return;
